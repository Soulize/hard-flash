...
其中最有工程意义的信号是：轨迹不等于质量；UP 和 UP2 使用相同的 preset 形态并表现出几乎相同的语言指纹，但报告分数分别为 85.7 和 42.9。
...
在这次对比记录中，同一段 hard-flash 提示词相较于轻量配置大约使用了 1.5 倍工作时间、5 倍输入 token 和 3 倍输出 token。
